...
<head>
    ...
    <style>
        ...
        .proposal-details { background: #f8f9fa; border: 2px solid #e9ecef; border-radius: 10px; padding: 20px; margin: 30px 0; }
        .proposal-details h3 { margin: 0 0 10px 0; color: #3a7bb3; font-size: 1.1rem; }
        .proposal-details p { margin: 0 0 10px 0; }
...
<body>
    <div class="container">
        <div class="header">
            ...
            <p>New Exchange Proposal</p>
        </div>
        <div class="content">
            <h2>Hello {{ $user->name }}!</h2>
            <p>You have received a new exchange proposal from <strong>{{ $exchange->initiator->name }}</strong> on SkillSwap.</p>
            <div class="proposal-details">
                <h3>Proposal Details</h3>
                <p><strong>Title:</strong> {{ $exchange->title }}</p>
                <p><strong>Description:</strong> {{ $exchange->description }}</p>
            </div>
            <a href="{{ $actionUrl }}" class="btn">View Proposal</a>
